...
# ===== Demo အား/မအား စစ် (တိကျအောင် ပြင်) =====
def check_demo_available():
    try:
        ...
        # Result ရရင် Demo အား
        if result:
            return True
        return False
    except Exception as e:
        err = str(e)
        # Queue / Busy / Error ဆိုရင် Demo မရ
        print(f"[Demo Check] {err[:200]}")
        return False
...
# ===== Colab Server သုံး (Ngrok Header ပါ) =====
def run_tts_server(text, output_path, ref_audio_path=None, progress_callback=None):
    ...
    # ===== Ngrok Warning ဖျောက်ဖို့ Header =====
    headers = {
        "ngrok-skip-browser-warning": "true",
        "User-Agent": "MyStreamlitApp/1.0"
    }
...
def run_tts_chunked(text, output_path, ref_audio_path=None, progress_callback=None):
    ...
    if demo_ok:
        ...
        if not VOXCPM_SERVER:
            raise Exception("❌ Demo Busy + Server URL မထည့်ရသေးပါ။ Streamlit Secrets မှာ VOXCPM_SERVER ထည့်ပါ။")
