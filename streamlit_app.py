...
if video_file is not None:
    if st.button("🚀 Generate Recap", type="primary"):
        ...
        with st.spinner("✍️ Gemini → Script ရေးနေသည်..."):
            ...
            words_needed = int(video_duration * 2.5)
            prompt = (
                f"Watch this video carefully. It is {video_duration:.1f} seconds long. "
                f"Write a movie recap narration script in {LANGUAGE} language. "
                f"IMPORTANT: The script must take EXACTLY {video_duration:.0f} seconds "
                f"to narrate at normal Burmese speaking speed. "
                f"Aim for approximately {words_needed} Burmese characters (NOT words). "
                f"Do NOT exceed this length. Return plain speech text only."
            )
